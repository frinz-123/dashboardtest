...
export default function GoalDetailsDialog({
    ...
    const chartData = useMemo(() => {
        ...
        for (let i = 0; i < totalDays; i++) {
            ...
            data.push({
                date: currentDate.toLocaleDateString('es-ES', { day: 'numeric', month: 'short' }),
                fullDate: dateKey,
                ...
                <div className="grid gap-4 py-4">
                    <div className="grid grid-cols-3 gap-4 text-center">
                        <div className="rounded-lg border bg-card p-3 shadow-sm">
                            <div className="text-[10px] uppercase text-muted-foreground font-medium">Meta Total</div>
                            <div className="text-lg font-bold">{formatCurrency(goal)}</div>
                        </div>
                        <div className="rounded-lg border bg-card p-3 shadow-sm">
                            <div className="text-[10px] uppercase text-muted-foreground font-medium">Actual</div>
                            <div className="text-lg font-bold text-blue-600">{formatCurrency(currentSales)}</div>
                        </div>
                        <div className="rounded-lg border bg-card p-3 shadow-sm">
                            <div className="text-[10px] uppercase text-muted-foreground font-medium">Proyección</div>
                            <div className={`text-lg font-bold ${statusColor}`}>{formatCurrency(projection)}</div>
                        </div>
                    </div>
                        ...
                        <ResponsiveContainer width="100%" height="100%">
                            <ComposedChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                                ...
                                <Tooltip
                                    ...
                                    formatter={(value: number, name: string) => [formatCurrency(value), name === 'ideal' ? 'Meta Ideal' : 'Venta Acumulada']}
