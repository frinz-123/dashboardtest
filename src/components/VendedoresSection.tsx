...
export default function VendedoresSection({
  ...
  const selectedSellerData = useMemo(() => {
    ...
    return filteredSellers.find(seller => seller.vendedor === selectedSeller) ?? null
          ...
          <div className="mt-2 pt-2 border-t border-gray-100">
            <div className="flex flex-wrap gap-2 text-xs">
              ...
              <span className="bg-gray-100 text-gray-600 px-2 py-1 rounded">
                {filteredSellers.length} vendedor{filteredSellers.length !== 1 ? 'es' : ''} disponible{filteredSellers.length !== 1 ? 's' : ''}
              </span>
            </div>
          </div>
        )}
        ...
        <div className="space-y-3">
          <div className="bg-white rounded-lg p-4 border border-[#E2E4E9] flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-gray-500">Visualizando detalle de {selectedSellerData?.vendedor}</span>
            </div>
            {selectedSellerData && (
              <div className="flex items-center gap-2">
                <button
                  onClick={() => exportSellerData('clients', selectedSellerData.vendedor)}
                ...
                <button
                  onClick={() => exportSellerData('products', selectedSellerData.vendedor)}
                  className="text-xs border border-gray-300 rounded px-3 py-2 bg-white hover:bg-gray-50 flex items-center"
                >
                  <Download className="w-3 h-3 mr-2" />
                  Exportar productos
