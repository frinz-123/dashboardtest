...
export default function CleyOrderQuestion({
  onChange,
  value = "1",
}: CleyOrderQuestionProps) {
  const id = useId();

  const items = [
    { value: "1", label: "No, no levanté pedido" },
    { value: "2", label: "Si, Levanté pedido" },
  ];

  return (
    <fieldset className="space-y-4">
      <legend className="text-foreground text-sm leading-none font-medium">
        Levantaste pedido en Casa Ley?
      </legend>
      <RadioGroup
