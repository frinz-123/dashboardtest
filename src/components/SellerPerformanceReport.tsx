...
export default function SellerPerformanceReport({
  ...
  const avgSales =
    sellers.reduce((sum, s) => sum + s.totalSales, 0) / sellers.length;
  const avgTicket =
  ...
  const topBySales = [...sellers]
    .sort((a, b) => b.totalSales - a.totalSales)
    .slice(0, 3);
  const topByVelocity = [...sellers]
    .sort(
      (a, b) => b.salesVelocity.velocityScore - a.salesVelocity.velocityScore,
    )
    .slice(0, 3);
  const topByRetention = [...sellers]
    .sort(
      (a, b) =>
        b.retentionAnalysis.retentionRate - a.retentionAnalysis.retentionRate,
    )
    .slice(0, 3);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-lg p-4 border border-[#E2E4E9]">
        <h3 className="font-semibold text-gray-700 mb-3 flex items-center text-sm">
          <TrendingUp className="mr-2 h-4 w-4 text-green-500" /> Top 3 - Ventas
          Totales
        </h3>
          ...
          {topBySales.map((seller, index) => (
            ...
            >
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">
                    {seller.uniqueClients} clientes • {seller.totalVisits}{" "}
                    visitas
                  </p>
                </div>
              </div>
              <div className="text-right">
                ...
                <p className="text-xs text-gray-500">
                  {((seller.totalSales / avgSales - 1) * 100).toFixed(0)}% vs
                  promedio
                </p>
              </div>
            </div>
      ...
      <div className="bg-white rounded-lg p-4 border border-[#E2E4E9]">
        <h3 className="font-semibold text-gray-700 mb-3 flex items-center text-sm">
          <Clock className="mr-2 h-4 w-4 text-blue-500" /> Top 3 - Velocidad de
          Ventas
        </h3>
        <div className="space-y-2">
      ...
      <div className="bg-white rounded-lg p-4 border border-[#E2E4E9]">
        <h3 className="font-semibold text-gray-700 mb-3 flex items-center text-sm">
          <Users className="mr-2 h-4 w-4 text-purple-500" /> Top 3 - Retención
          de Clientes
        </h3>
        <div className="space-y-2">
      ...
      <div className="bg-white rounded-lg p-4 border border-[#E2E4E9]">
        <h3 className="font-semibold text-gray-700 mb-3 flex items-center text-sm">
          <Calendar className="mr-2 h-4 w-4" /> Análisis de Días de Mejor
          Rendimiento
        </h3>
        <div className="grid grid-cols-2 md:grid-cols-7 gap-2">
          {[
            ...
            const sellersWithBestDay = sellers.filter(
              (s) => s.salesVelocity.peakAnalysis.bestSalesDay === day,
            );
            const count = sellersWithBestDay.length;
            const percentage =
              sellers.length > 0 ? (count / sellers.length) * 100 : 0;
        ...
        <p className="text-xs text-gray-500 mt-2">
          Número de vendedores que tienen su mejor día de ventas en cada día de
          la semana
        </p>
      </div>
    </div>
