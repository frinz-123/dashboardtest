...
export function useSubmissionQueue(): UseSubmissionQueueReturn {
            ...
            try {
                const response = await fetch("/api/submit-form", {
                    ...
                    body: JSON.stringify({
                        ...submission.payload,
                        queuedAt:
                            submission.payload.queuedAt ?? submission.createdAt,
    ...
    const uploadPhoto = useCallback(
        async (photoId: string, submissionId: string): Promise<string> => {
            ...
            if (!response.ok) {
                const message =
                    data?.error || `Upload error: ${response.status}`;
        ...
        async (item: QueuedSubmission): Promise<boolean> => {
            ...
            if (result.success) {
                ...
                if (finalItem.payload.photoIds?.length) {
                    deletePhotos(finalItem.payload.photoIds).catch((error) => {
                        console.error(
                            "Failed to delete uploaded photos:",
                            error,
                        );
                    });
                }
            ...
            if (newRetryCount >= MAX_RETRIES) {
                ...
                await submissionQueue.update(item.id, {
                    ...
                    retryCount: newRetryCount,
                    errorMessage:
                        result.error || "Maximo de reintentos alcanzado",
                });
                haptics.error();
                return false;
            }
    ...
    const shouldAutoRetryFailedItem = useCallback((item: QueuedSubmission) => {
        ...
        if (
            !/ubicaci|location|gps|precisi|lejos|distance/i.test(
                item.errorMessage,
            )
        )
            return false;
            ...
            for (const item of items) {
                ...
                if (!success && item.retryCount < MAX_RETRIES) {
                    ...
                    const delay = Math.min(
                        RETRY_DELAY_BASE * Math.pow(2, item.retryCount),
                        16000,
                    );
    ...
    useEffect(() => {
        const handleOnline = async () => {
            ...
            const syncRegistered =
                await submissionQueue.requestBackgroundSync();
            if (!syncRegistered) {
                console.log(
                    "Background Sync not available, processing manually...",
                );
                processQueue();
            }
        };
        ...
        const handleSWMessage = (event: MessageEvent<SWMessage>) => {
            ...
            switch (type) {
                ...
                case "SUBMISSION_FAILED":
                    console.log(
                        `❌ [SW] Submission ${submissionId} failed:`,
                        error,
                    );
                    haptics.error();
                    loadQueue();
