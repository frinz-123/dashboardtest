...
export async function POST(req: Request) {
    ...
    try {
        ...
        console.log("🔍 FORM SUBMISSION RECEIVED:", {
            ...
            locationAgeSeconds:
                locationAge !== null
                    ? Number((locationAge / 1000).toFixed(1))
                    : null,
            queuedAt,
            queuedAgeMs: queuedAge,
            queuedAgeSeconds:
                queuedAge !== null
                    ? Number((queuedAge / 1000).toFixed(1))
                    : null,
        ...
        if (
            ...
            if (shouldBypassLocationChecks) {
                ...
                console.error(
                    "❌ LOCATION VALIDATION FAILED: Poor GPS accuracy",
                    {
                        accuracy: location.accuracy,
                        maxAllowed: MAX_LOCATION_ACCURACY,
                        timestamp: new Date().toISOString(),
        ...
        if (!isAdmin && location.timestamp !== undefined) {
            ...
            if (locationAge > MAX_LOCATION_AGE) {
                if (allowQueuedStaleLocation) {
                    ...
                    console.error(
                        "❌ LOCATION VALIDATION FAILED: Stale location",
                        {
                            locationAge,
                            maxAllowed: MAX_LOCATION_AGE,
                            locationTimestamp: location.timestamp,
        ...
        try {
            ...
            if (clientLat && clientLng) {
                ...
                if (!isNaN(clientLatNum) && !isNaN(clientLngNum)) {
                    ...
                    if (isAdmin && distanceToClient > MAX_CLIENT_DISTANCE) {
                        console.log(
                            "👤 ADMIN OVERRIDE: Bypassing distance validation",
                            {
                                distance: distanceToClient,
                                maxAllowed: MAX_CLIENT_DISTANCE,
                                userEmail,
            ...
            const range = `Form_Data!A${lastRow}:AP${lastRow}`;
            ...
            const rawOffsetHours = offsetMatch
                ? parseInt(offsetMatch[1], 10)
                : 0;
            const offsetMinutes =
                offsetMatch && offsetMatch[2]
            ...
            const periodSourceDate = Number.isNaN(
                mazatlanDateForPeriod.getTime(),
            )
                ? submissionTimestamp
            ...
            if (isAdmin && overrideMonthCode && overrideMonthCode.trim()) {
                ...
                console.log(
                    "📅 ADMIN MONTH CODE OVERRIDE: Using override month code",
                    {
                        overrideMonthCode,
                        adminEmail: adminEmailForValidation,
                        timestamp: new Date().toISOString(),
                ...
                const myParts =
                    monthYearFormatter.formatToParts(periodSourceDate);
                const mPart =
                    myParts.find((p) => p.type === "month")?.value || "";
                const yPart =
                    myParts.find((p) => p.type === "year")?.value || "";
                monthYearCode = `${mPart}_${yPart}`
                    .toUpperCase()
                    .replace(".", "");
            }
