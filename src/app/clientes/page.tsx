...
function ClientesDesatendidos({
    ...
    const vendedorOptions = useMemo(() => {
        ...
        const all = Object.values(analyticsData.clientVendedores).filter(
            Boolean,
        );
        ...
        const rows = Object.entries(analyticsData.clientStats)
            .map(([client, stats]) => {
                ...
                const conversionRate =
                    stats.entries > 0
                        ? (stats.salesEntries / stats.entries) * 100
                        : 0;
                return {
                    client,
                    code,
        ...
        const vendedorFiltered =
            selectedVendedores.length > 0
                ? codeFiltered.filter((r) => {
                      const vendedor =
                          analyticsData?.clientVendedores?.[r.client];
            ...
            <div className="mb-4 flex items-start justify-between gap-4">
                <div>
                    ...
                    <p className="text-xs text-gray-500">
                        Visitas incluye todas las visitas (incluso sin venta).
                        Conv. = % de visitas con venta.
                    </p>
                    <div className="mt-2 flex flex-wrap gap-2">
                        {printFilters.map((filter) => (
                            <span
                                key={filter.label}
                                className="print-badge text-[10px]"
                            >
            ...
            {items.length === 0 ? (
                ...
                <table className="print-table text-[11px]">
                    ...
                    <tbody>
                        {items.map((row) => {
                            const vendedor =
                                analyticsData?.clientVendedores?.[row.client] ||
                                "Sin vendedor";
                            return (
                ...
                <div className="flex items-start justify-between gap-3">
                    <div>
                        ...
                        <p className="text-xs text-gray-500 mt-1">
                            Visitas incluye todas las visitas (incluso sin
                            venta). Conv. = % de visitas con venta.
                        </p>
                    </div>
                    <button
                        type="button"
                ...
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    <div className="flex items-center space-x-1">
                        ...
                        >
                            ...
                            <option value="ultimaVisita">
                                Última visita más antigua
                            </option>
                        </select>
                    </div>
                    ...
                    <div className="space-y-2">
                        {visible.map((row) => {
                            const needsAttention =
                                row.lastDays >= 60 || row.entries <= 1;
                            const lowConversion =
                                row.conversionRate < 50 && row.entries >= 3;
                            return (
                                <div
                                    key={row.client}
...
function ProductosPorCodigo({
    ...
    const flattenedRows = useMemo(() => {
        ...
        const allProductsAllCodes = Array.from(
            new Set(
                Object.values(productsByCode).flatMap((map) =>
                    Object.keys(map || {}),
                ),
            ),
        ).sort();
        ...
        filteredCodes.forEach((code) => {
            ...
            const codeTotal = Object.values(map).reduce(
                (s, q) => s + (q || 0),
                0,
            );
        ...
        rows.sort((a, b) => {
            ...
            if (sortBy === "shareCode")
                return b.percentOfCode - a.percentOfCode;
            return 0;
        });
        ...
        <div>
            <div className="mb-3">
                <div className="flex items-center justify-between">
                    <h3 className="font-semibold text-gray-700 flex items-center text-sm">
                        <Package className="mr-2 h-4 w-4" /> Productos por
                        código
                    </h3>
...
export default function ClientesPage() {
    ...
    const [entriesFilter, setEntriesFilter] = useState<
        "all" | "recent" | "month"
    >("all");
    const [viewMode, setViewMode] = useState<"dashboard" | "client">(
        "dashboard",
    );
    ...
    const [productosPorCodigoDateFilter, setProductosPorCodigoDateFilter] =
        useState<"currentMonth" | "30d" | "2m" | "6m" | "year" | "custom">(
            "year",
        );
    const [
    ...
    const fetchProductosPorCodigoData = async (
        ...
        try {
            console.log(
                "🔍 Fetching productos por codigo data with date filters:",
                {
                    dateFrom,
                    dateTo,
                },
            ...
            const data = await response.json();
            console.log(
                "📊 Frontend received productos por codigo data:",
                data,
            );
            if (data.success) {
                ...
                console.error(
                    "❌ Error fetching productos por codigo:",
                    data.error,
                );
            }
        } catch (error) {
            console.error(
                "🚨 Error fetching productos por codigo data:",
                error,
            );
        } finally {
    ...
    const fetchClientesDesatendidosData = async (
        ...
        try {
            console.log(
                "🔍 Fetching clientes desatendidos data with date filters:",
                {
                    dateFrom,
                    dateTo,
                },
            ...
            const data = await response.json();
            console.log(
                "📊 Frontend received clientes desatendidos data:",
                data,
            );
            if (data.success) {
                console.log(
                    "✅ Setting clientes desatendidos data:",
                    data.data,
                );
                setClientesDesatendidosData(data.data);
            } else {
                console.error(
                    "❌ Error fetching clientes desatendidos:",
                    data.error,
                );
            }
        } catch (error) {
            console.error(
                "🚨 Error fetching clientes desatendidos data:",
                error,
            );
        } finally {
    ...
    const fetchSellerAnalyticsData = async (
        ...
        try {
            console.log(
                "👥 Fetching seller analytics data with date filters:",
                {
                    dateFrom,
                    dateTo,
                },
                ...
                const transformedData = {
                    ...
                    topSellerName:
                        data.data.sellers.length > 0
                            ? data.data.sellers[0].vendedor
                            : "",
                    topSellerSales:
                        data.data.sellers.length > 0
                            ? data.data.sellers[0].totalSales
                            : 0,
                };
                setSellerAnalyticsData(transformedData);
            } else {
                console.error(
                    "❌ Error fetching seller analytics:",
                    data.error,
                );
                setError(
                            ...
                            {debouncedSearchTerm &&
                                filteredClients.length === 20 && (
                                    <div className="px-4 py-2 text-xs text-gray-500 italic">
                                        ...
                                        Continúa escribiendo para refinar la
                                        búsqueda.
                                    </div>
                                )}
                        </div>
                ...
                <div className="space-y-3">
                    ...
                    <div className="bg-white rounded-lg p-4 border border-[#E2E4E9]">
                        ...
                        <div className="mb-3 space-y-2">
                            <div className="flex flex-wrap gap-2">
                                <button
                                    onClick={() =>
                                        setTopClientsDateFilter("currentMonth")
                                    }
                                    className={`px-3 py-1.5 text-xs rounded-md transition-colors ${
                                ...
                                <button
                                    onClick={() =>
                                        setTopClientsDateFilter("custom")
                                    }
                                    className={`px-3 py-1.5 text-xs rounded-md transition-colors ${
                    ...
                    <div className="bg-white rounded-lg p-4 border border-[#E2E4E9]">
                        ...
                        <div className="mb-3 space-y-2">
                            <div className="flex flex-wrap gap-2">
                                <button
                                    onClick={() =>
                                        setTopProductsDateFilter("currentMonth")
                                    }
                                    className={`px-3 py-1.5 text-xs rounded-md transition-colors ${
                                ...
                                <button
                                    onClick={() =>
                                        setTopProductsDateFilter("custom")
                                    }
                                    className={`px-3 py-1.5 text-xs rounded-md transition-colors ${
                    ...
                    <div className="bg-white rounded-lg p-4 border border-[#E2E4E9]">
                        <h3 className="font-semibold text-gray-700 mb-3 flex items-center text-sm">
                            <TrendingUp className="mr-2 h-4 w-4" /> Tendencia
                            Mensual
                        </h3>
                        <div className="space-y-2">
                            {isLoadingAnalytics ? (
...
function InsightsList({
    ...
    const avg = (arr: ClientEntry[]) =>
        arr.length
            ? arr.reduce((s, e) => s + (e.total || 0), 0) / arr.length
            : 0;
        ...
        <div className="space-y-4">
            ...
            <div className="p-3 border rounded-md">
                <p className="font-medium text-sm text-gray-800 mb-1">
                    Productos que pudieron dejar de venderse (≥ {DAYS_THRESHOLD}{" "}
                    días)
                </p>
            ...
            <div className="p-3 border rounded-md">
                ...
                <p className="text-xs text-gray-600">
                    ...
                    {formatCurrency(previousAvg)} · Cambio:{" "}
                    {basketDelta.toFixed(1)}%
                </p>
            </div>
