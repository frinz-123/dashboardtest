...
import {
  ...
  isCleyClient as checkIsCleyClient,
  DEFAULT_START_LAT,
  DEFAULT_START_LNG,
  debug,
  getMazatlanDayName,
...
export default function RecorridosPage() {
  ...
  const fetchClients = async (
    ...
    try {
      ...
      const useVendor =
        overrideVendor !== undefined ? overrideVendor : selectedVendorEmail;
  ...
  const fetchConfiguration = async () => {
    try {
      const response = await fetch(
        buildApiUrl("configuracion", userEmail, false, null),
      );
      ...
      if (data.data?.length > 0) {
        ...
        setConfig((prev) => ({
          ...
          tiempoPromedioVisita:
            parseInt(configMap.tiempo_promedio_visita, 10) ||
            prev.tiempoPromedioVisita,
          maximoClientesPorDia:
            parseInt(configMap.maximo_clientes_dia, 10) ||
      ...
      if (data.data?.length > 0) {
        ...
        data.data.forEach((record: any) => {
          if (
            ...
            const weekNumber = parseInt(record.semana, 10);
      ...
      if (data.data?.length > 0) {
        ...
        data.data.forEach((record: any) => {
          if (
            ...
            record.estado !== "Programado"
          )
            return;
          const clientName = record.cliente_nombre;
          const scheduledDateOnly =
            typeof record.proxima_visita_programada === "string" &&
          ...
          const classify = (d: string) =>
            d === mazatlanToday ? 0 : d < mazatlanToday ? 1 : 2;
          const shouldReplace =
            !existing?.scheduledDate ||
            classify(scheduledDateOnly) < classify(existing.scheduledDate) ||
            (classify(scheduledDateOnly) === classify(existing.scheduledDate) &&
              (classify(scheduledDateOnly) === 1
                ? scheduledDateOnly > existing.scheduledDate
                : scheduledDateOnly < existing.scheduledDate));
          if (shouldReplace) {
            visits[clientName] = {
              clientName,
              scheduledDate: scheduledDateOnly,
              week: record.semana_numero
  ...
  const loadPersistedRouteState = (
    ...
    try {
      ...
      const matchingRecord = dataToCheck.find((record) => {
        ...
        const isCorrectVendor = vendorAliases.has(
          normalizeVendorValue(String(record.vendedor || "")),
        );
      ...
      if (matchingRecord) {
        ...
        setRouteInProgress(false);
        saveRouteState(
          matchingRecord.tiempo_inicio,
          true,
          matchingRecord.tiempo_fin,
          dayToSearch,
        );
        return;
      }
  ...
  const checkRouteInProgress = () => {
    try {
      ...
      const hasCompletedVisitsToday =
        isSelectedDayToday &&
        Object.values(visitHistory).some(
  ...
  const expandCleyClients = (clients: Client[]): Client[] => {
    ...
    clients.forEach((client) => {
      if (checkIsCleyClient(client.Tipo_Cliente) && client.Entrega) {
        const pedidosDay =
          rescheduledClients[`${client.Nombre} (Pedidos)`]?.newDay ||
          client.Dia;
        const entregaDay =
          rescheduledClients[`${client.Nombre} (Entrega)`]?.newDay ||
          client.Entrega;
        expanded.push({
          ...client,
          Nombre: `${client.Nombre} (Pedidos)`,
          Dia: pedidosDay,
        });
        expanded.push({
          ...client,
          Nombre: `${client.Nombre} (Entrega)`,
          Dia: entregaDay,
        });
      } else {
  ...
  const shouldBeConsideredForVisitToday = (client: Client): boolean => {
    ...
    const lastVisitDateObj = new Date(
      `${clientHistory.lastVisitDate}T12:00:00.000Z`,
    );
    const daysSinceLastVisit = Math.floor(
      (mazatlanTime.getTime() - lastVisitDateObj.getTime()) /
        (1000 * 60 * 60 * 24),
    );
  ...
  const updateVisitStatus = async (
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (status === "completed") {
          const fecha =
            responseData?.data?.fecha || getSelectedDayDate(selectedDay);
  ...
  const finishRoute = async () => {
    ...
    try {
      ...
      const actualRouteDistance = calculateTotalRouteDistance(
        allCompletedTodayClients,
      );
      ...
      const skippedClients = clientsConsideredForVisitToday
        .filter(
          (c) =>
            visitStatus[c.Nombre] === "skipped" ||
            visitStatus[c.Nombre] === "postponed",
        )
        .map((c) => c.Nombre);
  ...
  const rescheduleFromPostponePool = (
    ...
    setRescheduledClients((prev) => ({
      ...
      [clientName]: {
        originalDay: postponedClient.originalDay,
        newDay: targetDay,
        visitType: isCley ? visitType : "Normal",
      },
    }));
    setPostponePool((prev) => {
      const p = { ...prev };
  ...
  const expandedClients = useMemo(
    () => expandCleyClients(clients),
    [clients, expandCleyClients],
  );
  const clientsForSelectedDay = useMemo(
    () =>
      expandedClients.filter(
        (c) => normalizeDay(c.Dia || "") === normalizeDay(selectedDay),
      ),
  ...
  const postponedClientsForToday = useMemo(
    () =>
      Object.values(postponePool)
        .filter(
          (p) => normalizeDay(p.client.Dia || "") === normalizeDay(selectedDay),
        )
        .map((p) => p.client),
  ...
  const actuallyPendingClients = useMemo(
    ...
    [
      clientsConsideredForVisitToday,
      visitHistory,
      visitStatus,
  ...
  const allCompletedTodayClients = useMemo(
    () =>
      allClientsForSelectedDay.filter(
        (c) =>
          visitHistory[getOriginalClientName(c.Nombre)]?.lastVisitDate ===
          selectedDayDateString,
      ),
  ...
  const getVisitStatusText = (client: Client): string => {
    ...
    if (scheduledVisit?.status === "Programado") {
      const daysDiff = Math.floor(
        (new Date(scheduledVisit.scheduledDate).getTime() -
          new Date(mazatlanToday).getTime()) /
          (1000 * 60 * 60 * 24),
      );
    ...
    const daysSince = Math.floor(
      (Date.now() - new Date(history.lastVisitDate).getTime()) /
        (1000 * 60 * 60 * 24),
    );
    if (daysSince === 0) return "Completado hoy";
    const weeksSince = Math.floor(daysSince / 7);
    return weeksSince === 0
      ? `Visitado hace ${daysSince} días`
      : `Visitado hace ${weeksSince} semanas`;
  };
    ...
    <>
      ...
      >
        <AppHeader
          title="Recorridos"
          icon={Route}
          subtitle={isMaster ? viewingAsVendor : undefined}
        />
        ...
        <main className="px-4 py-4 max-w-2xl mx-auto space-y-4">
          ...
          <VendorSelector
            ...
            onVendorChange={(email) => {
              ...
              setViewingAsVendor(
                email ? getVendorLabel(email) : "Todas las Rutas",
              );
            }}
            isVisible={isMaster}
          />
            ...
            <div className="p-5">
              <div className="flex items-start justify-between mb-5">
                <div>
                  ...
                  <p className="text-sm text-slate-500 mt-0.5">
                    {getSelectedDayFormattedDate(selectedDay)}
                  </p>
                </div>
                {/* Circular Progress */}
                  ...
                  <div className="flex items-center gap-4 text-sm">
                    <div className="flex items-center gap-1.5 text-blue-700">
                      ...
                      <span className="font-medium">
                        {totalRouteDistance.toFixed(1)} km
                      </span>
                    </div>
                    <div className="flex items-center gap-1.5 text-blue-700">
              ...
              {Object.keys(postponePool).length > 0 && (
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 py-2.5 bg-amber-50 hover:bg-amber-100 text-amber-700 font-medium rounded-xl transition-colors border border-amber-200/50 mb-4"
                >
                  <AlertTriangle className="h-4 w-4" />
                  {showPostponePool ? "Ocultar" : "Ver"} Pospuestos (
              ...
              {pendingReschedules.length > 0 && (
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 py-3 bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 text-white font-medium rounded-xl transition-all disabled:opacity-50 shadow-lg shadow-orange-500/20 mb-4"
                >
                  {savingReschedules ? (
                ...
                <div className="flex items-center gap-3 p-4 bg-gradient-to-r from-emerald-50 to-green-50 rounded-xl border border-emerald-200/50 mb-4">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-emerald-600">
                      Puedes finalizar nuevamente si es necesario
                    </div>
                  </div>
                </div>
              )}
            ...
            <div className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-2xl card-shadow border border-amber-200/50 p-4 animate-slideUp">
              ...
              <div className="space-y-3">
                {Object.entries(postponePool).map(([clientName, data]) => (
                  <div
                    key={clientName}
                    className="bg-white rounded-xl p-4 border border-amber-200/50"
                  >
                    <div className="flex items-center justify-between mb-3">
                      <div>
                      ...
                      <span
                        className={`px-2.5 py-1 rounded-lg text-xs font-semibold border ${getTypeStyles(data.client.Tipo_Cliente)}`}
                      >
                        {data.client.Tipo_Cliente}
                      </span>
                    </div>
                    <div className="grid grid-cols-7 gap-1">
                      {ROUTE_DAYS.map((day) => (
            ...
            {loading ? (
              ...
              <div className="bg-white rounded-2xl card-shadow border border-slate-200/50 p-12 text-center">
                ...
                <p className="text-sm text-slate-500">
                  No hay visitas programadas para {selectedDay}
                </p>
              </div>
            ) : (
              optimizedPendingClients.map((client, index) => {
                ...
                const distanceFromPrevious =
                  index === 0
                    ...
                    : calculateDistance(
                        optimizedPendingClients[index - 1].Latitude,
                        optimizedPendingClients[index - 1].Longitud,
                        client.Latitude,
                        client.Longitud,
                      );
                const originalName = getOriginalClientName(client.Nombre);
                ...
                return (
                  <div
                    key={index}
                    className={`bg-white rounded-2xl card-shadow card-shadow-hover border border-slate-200/50 overflow-hidden transition-all duration-300 animate-slideUp ${isCompleting ? "scale-[0.98] opacity-70 border-emerald-300 bg-emerald-50" : ""}`}
                  ...
                  >
                    <div className="p-4">
                      <div className="flex items-start gap-3 mb-4">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 flex-wrap mb-1">
                            ...
                            <span
                              className={`px-2 py-0.5 rounded-md text-xs font-semibold border ${getTypeStyles(client.Tipo_Cliente)}`}
                            >
                              {client.Tipo_Cliente}
                            </span>
                            {visitType !== "Normal" && (
                              <span
                                className={`px-2 py-0.5 rounded-md text-xs font-semibold border ${visitType === "Pedidos" ? "bg-orange-50 text-orange-700 border-orange-200" : "bg-purple-50 text-purple-700 border-purple-200"}`}
                              >
                                {visitType}
                              </span>
                            )}
                      ...
                      <div className="flex gap-2">
                        ...
                        >
                          {isCompleting ? (
                            <CheckCircle className="h-4 w-4 animate-checkmark" />
                          ) : null}
                            ...
                            <div className="grid grid-cols-3 gap-1 mb-1">
                              {ROUTE_DAYS.slice(0, 6).map((day) => (
                                <button
                                  key={day}
                                  onClick={() => {
                                    postponeClient(client, day);
                                    setCleyDaySelector(null);
                                  }}
                                  className="py-1 text-xs font-medium bg-amber-100 text-amber-700 rounded-lg hover:bg-amber-200"
                                >
                                  {day.slice(0, 2)}
                                </button>
                              ))}
                        ...
                        <button
                          ...
                          className="w-11 h-11 rounded-xl bg-blue-100 text-blue-600 hover:bg-blue-200 flex items-center justify-center transition-colors"
                        >
          ...
          {allCompletedTodayClients.length > 0 && (
            <div className="bg-white rounded-2xl card-shadow border border-slate-200/50 overflow-hidden">
              ...
              >
                ...
                <ChevronDown
                  className={`h-5 w-5 text-slate-400 transition-transform ${showCompletedSection ? "rotate-180" : ""}`}
                />
              </button>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500">
                            Próxima en {client.Frecuencia} sem
                          </p>
                        </div>
                      </div>
                      <span
